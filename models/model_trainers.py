...
class RailTrackModelComparison:
    ...
    def train_detectron2(self) -> dict:
        ...
        # Evaluate
        evaluator = COCOEvaluator("rail_tracks_val", cfg, False, output_dir=cfg.OUTPUT_DIR)
    ...
    def benchmark_inference(self, num_images: int = 20) -> dict:
        ...
        test_images = list((self.version_dir / "yolov8/test/images").glob("*.jpg"))[:num_images]
        ...
        for model_name in ["yolov8", "detectron2"]:
            ...
            if model_name == "yolov8":
                model = YOLO(str(self.output_dir / "yolov8_rail_tracks/weights/best.pt"))
                ...
                cfg = get_cfg()
                cfg.merge_from_file(str(self.output_dir / "detectron2_rail_tracks/config.yml"))
                ...
                for img_path in test_images:
                    ...
                    self.metrics[model_name]["predictions"].append({
                        "image": str(img_path),
                        "boxes": prediction["instances"].pred_boxes.tensor.cpu().numpy().tolist()
                    })
            
            results[model_name] = self._measure_resources(start_time, start_memory)
            results[model_name]["images_per_second"] = num_images / results[model_name]["time_seconds"]
    ...
    def save_results(self):
        ...
        results_path = self.output_dir / f"rail_track_detection_comparison_{timestamp}.json"
    ...
    def run_comparison(self):
        ...
        print("\nInference Metrics:")
        print(f"YOLOv8:")
        print(f"- Speed: {inference_metrics['yolov8']['images_per_second']:.2f} images/second")
        print(f"- Memory: {inference_metrics['yolov8']['memory_mb']:.1f}MB")
        
        print("\nDetectron2:")
        print(f"- Speed: {inference_metrics['detectron2']['images_per_second']:.2f} images/second")
